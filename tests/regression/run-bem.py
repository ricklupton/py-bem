...
def find_solution(model, windspeed, rotorspeed, pitch):
    # Along blade
    factors = model.solve(windspeed, rotorspeed, pitch)
    forces = model.forces(windspeed, rotorspeed, pitch, 1.225, factors)
    xdot = model.inflow_derivatives(windspeed, rotorspeed, pitch,
                                    factors)
    ...
    # Overall rotor
    pcoeffs = model.pcoeffs(windspeed, rotorspeed, pitch)
    print()
    print("Pcoeffs:")
    print()
    print("\t".join(['CT', 'CQ', 'CP']))
